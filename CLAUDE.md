...
## 컴포넌트 수정 시 스토리 동기화 규칙

기존 컴포넌트의 **스타일·props·동작을 바꾸면 반드시 같은 커밋에서** 그 컴포넌트의
`.stories.ts` 도 함께 고친다. 스토리는 문서이자 최소 테스트라, 어긋나는 순간
"문서가 거짓말하는 상태"가 된다.

체크리스트 — 변경 종류별로 스토리에서 손댈 곳:

| 바꾼 것 | 스토리에서 함께 고칠 곳 |
|---|---|
| 디자인 토큰(typo/color/spacing) | `parameters.docs.description.component` 의 **디자인 토큰** 섹션 |
| props 추가·삭제·기본값 변경 | `argTypes` 의 `description` / `table.defaultValue`, 관련 story `args` |
| DOM 구조·클래스명 | `play` 함수의 셀렉터, `.test.ts` 의 `querySelector` |
| 동작(이벤트·상태) | 해당 동작을 보여주는 story 추가 또는 `play` 보강 |
| 사용 가이드가 바뀜 | 컴포넌트 설명의 **컨텍스트별 사용 가이드** / **Copy 가이드** 표 |

- 토큰 값을 스토리 문서에 **하드코딩**해 둔 곳이 있으면 그것부터 확인한다.
  (`$body-small` 처럼 적어둔 문자열은 소스를 고쳐도 자동으로 따라오지 않는다)
- 변경 이유가 "왜"에 해당하면 문서에 인용문(`>`)으로 한 줄 남긴다.
  다음 사람이 같은 값을 되돌리지 않게 하기 위함이다.
- 스토리 없이 컴포넌트만 고친 PR 은 리뷰에서 되돌린다.
...
최신 표(컴포넌트별 사용 횟수 + 정책 외 추가분)는 Storybook → **Get Started → Migration** 페이지를 단일 출처로 한다. CLAUDE.md에는 정책만 둔다.
